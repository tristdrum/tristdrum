...
- When that separately authorized live path is enabled, it uses the shared durable Management/Ping sender with source `stock`. Ping retries never repeat WhatsApp or place an order. This capability does not enable the stock sender or override Tristan's prohibition on stock/grocery traffic; `airbnb-stock-management-alerts-10-40` remains inactive.
- Weekly review stores a suppressed `stock_count_review` record within the worker's household. The stock alert policy must permit this type alongside `stock_low` and `order_update`; the review itself sends no message. Integration tests cover its insert/upsert and household/service isolation.
- The observer reads the Airbnb Team and Airbnb Management groups as evidence only. A shortage message stores a hash and matched stock codes, never its raw body, and only marks those items as counts to confirm; it cannot move inventory or place an order.
