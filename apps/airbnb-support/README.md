...
- Contextual acceptance of a host's timing counteroffer uses the accepted offered clock, even when the guest does not repeat it. This room-timing path is independent of office-storage permission. A newly accepted conditional time creates the existing durable cleaner operation; unrelated courtesy and acknowledgements of an unchanged active arrangement do not invent another request.
