...
- Canonical knowledge includes the verified public Airbnb links for all three studios. When live availability is unknown, the adaptive agent offers relevant listing links instead of only promising to check, while leaving any real reservation decision unresolved. Links are not vacancy evidence or permission to change a booking. Management alerts carry a direct host-conversation link from the actual Airbnb thread ID when available; host-only links never belong in guest replies.
